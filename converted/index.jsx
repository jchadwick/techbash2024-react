    ...
    return (
        <CurrentUserProvider>
            <div className="flex flex-row">
                <h1 className="flex-grow text-4xl font-bold mb-8">
                    User Manager
                </h1>
                <CurrentUserView />
            </div>
...
function CurrentUserView() {
    ...
    if (!currentUser) {
        return (
            <div>
                <button onClick={() => authenticateUser({ username: 'admin', password: 'admin' })}>Login</button>
            </div>
        );
    }
...
function UsersList() {
    ...
    const handleUserSelected = React.useCallback((user) => {
    ...
    return (
        <div>
            ...
            <ul className="bg-white shadow rounded-lg divide-y divide-gray-200">
                {users.map((user) => (
                    <UserListItem key={user.id} user={user} onUserSelected={handleUserSelected} />
                ))}
            </ul>
...
function UserListItem({ user, onUserSelected }) {
    return (
        <li className="p-4 cursor-pointer" onClick={() => onUserSelected(user)}>
            {user.name}
        </li>
    )
}
